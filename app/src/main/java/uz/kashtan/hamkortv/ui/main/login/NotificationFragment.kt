...
class NotificationFragment : Fragment(R.layout.fragment_notification), OnItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        requestsDao = HTVDatabase.invoke(activity!!.applicationContext).requestsDao()
        val list = requestsDao.getRequests()
        adapter.setData(list)
        super.onViewCreated(view, savedInstanceState)
    }

    override fun onItemClick(requestId: String) {
        val model = requestsDao.getRequestById(requestId)
        if (!model.clicked) {
